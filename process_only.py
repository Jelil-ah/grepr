...
AI_DELAY = 1.0  # Seconds between AI calls (local is fast, no rate limit)
...
def find_latest_raw_file() -> Path | None:
    ...
    files = list(RAW_DIR.glob("raw_posts_*.json"))
    if not files:
        return None
...
def find_unprocessed_files() -> list[Path]:
    ...
    raw_files = set(RAW_DIR.glob("raw_posts_*.json"))
    processed_files = set(PROCESSED_DIR.glob("processed_posts_*.json"))
...
def main():
    ...
    if args.file:
        ...
        if not latest:
            logger.error("No raw_posts_*.json files found in data/raw/")
    ...
    for input_file in files_to_process:
        ...
        output_file = PROCESSED_DIR / f"processed_posts_{timestamp}_from_{input_file.stem}.json"
