...
def find_latest_processed_file() -> Path | None:
    ...
    files = list(PROCESSED_DIR.glob("processed_posts_*.json"))
    if not files:
        return None
...
def find_unpushed_files() -> list[Path]:
    ...
    processed_files = list(PROCESSED_DIR.glob("processed_posts_*.json"))
...
def main():
    ...
    if args.file:
        ...
        if not latest:
            logger.error("No processed_posts_*.json files found in data/processed/")
