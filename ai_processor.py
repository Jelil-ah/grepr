...
from config import (
    ...
    ETF_TICKERS, ETF_KEYWORDS
)


def detect_etf_mentions(text: str) -> list[str]:
    """
    Detect ETF tickers and keywords mentioned in text.
    Returns a list of detected ETF tickers/names.
    """
    if not text:
        return []
    ...
    # Check for exact ticker matches (case-insensitive but whole word)
    for ticker in ETF_TICKERS:
        # Use word boundaries to avoid false positives
        pattern = r'\b' + re.escape(ticker.lower()) + r'\b'
        if re.search(pattern, text_lower):
            detected.append(ticker.upper())

    # Check for keyword matches
    for keyword in ETF_KEYWORDS:
        if keyword.lower() in text_lower:
            # Add the keyword as detected (normalized)
            if keyword.lower() not in [d.lower() for d in detected]:
...
def get_ai_client():
    ...
    global _groq_client, _deepseek_client, _local_client

    if AI_PROVIDER == "local":
        # LM Studio local server (FREE - no API key needed)
        if _local_client is None:
            _local_client = OpenAI(
                api_key="not-needed",  # LM Studio doesn't require API key
                base_url=LOCAL_BASE_URL
            )
        return _local_client, LOCAL_MODEL, "local (LM Studio)"
...
def categorize_and_summarize(post: dict) -> dict:
    ...
    # Pre-detect ETF mentions before AI call
    full_text = f"{title} {content} {top_comment}"
    detected_etfs = detect_etf_mentions(full_text)
    post["etf_detected"] = detected_etfs

    # If ETFs detected, add hint to prompt
    etf_hint = ""
    if detected_etfs:
        etf_names = ", ".join(detected_etfs[:5])  # Limit to first 5
        etf_hint = f"\n\nATTENTION: Ce post mentionne des ETF/trackers: {etf_names}. Considère fortement la catégorie 'ETF'."
        print(f"  ETF detected: {etf_names}")
...
- IMPORTANT: Si le post mentionne des ETF/trackers (CW8, WPEA, MSCI World, S&P500, etc.), utilise "ETF"
    ...
    try:
        ...
        def make_api_call():
            return client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=2000,  # R1 models need more tokens for thinking
            )
        ...
        # 1. Remove <think>...</think> tags (DeepSeek R1 reasoning)
        ...
        # POST-PROCESSING: Force ETF category if ETF tickers detected
        # Only override if AI chose a generic category
        if detected_etfs:
            # Check if any actual ETF ticker (not just keyword) was detected
            ticker_matches = [etf for etf in detected_etfs if etf in [t.upper() for t in ETF_TICKERS]]
            if ticker_matches and post["category"] in ["Autre", "Epargne", "Strategie", "Question"]:
                print(f"  Overriding category '{post['category']}' -> 'ETF' (detected: {ticker_matches})")
                post["category"] = "ETF"
                # Add detected ETFs to tags if not present
                existing_tags = [t.lower() for t in post["tags"]]
                for ticker in ticker_matches[:3]:
                    if ticker.lower() not in existing_tags:
                        post["tags"].append(ticker)
